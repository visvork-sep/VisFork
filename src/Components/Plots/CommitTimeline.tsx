...
function CommitTimeline({ commitData,
    ...
    function groupNodes(data: Commit[]): MergedNode[] {
        ...
        for (let i = 1; i <= sorted.length; i++) {
            ...
            if (
                i === sorted.length ||
                commit.branch !== prevCommit.branch ||
                ...
                for (const mnode of mergedNodes) {
                    if (
                        mnode.branch === firstCommit.branch &&
                ...
                if (candidate) {
                    ...
                    const newMerged: MergedNode = {
                        id: `${firstCommit.repo} - ${firstCommit.branch}`,
                        parentIds: newParentIds,
                        repo: firstCommit.repo,
                        branch: firstCommit.branch,
    ...
    useEffect(() => {
        ...
        try {
            dag = builder(merged ? groupNodes(commitData) as MergedNode[] : commitData as Commit[]);
        } catch (error) {
        ...
        if (merged) {
            const mergedNodes = allNodes as unknown as d3dag.MutGraphNode<MergedNode, undefined>[];
            ...
            const regularCircles = allNodes.filter(node => node.data.branch !== defaultBranches[node.data.repo]);
            const regularTriangles = allNodes.filter(node => node.data.branch === defaultBranches[node.data.repo]);
            ...
            selection
                .on("mouseover", (event, d) => {
                    ...
                    if (!merged) {
                        ...
                        <strong>Branch</strong>: ${mergedData.branch}<br>
